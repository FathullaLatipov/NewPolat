...
class Zapchast_all(APIView):
    ...
    def put(request):
        ...
        required = ['name_uz', 'name_ru', 'status', "image"]
        arr = {}
        for i in required:
            if i not in request.data:
                return JsonResponse({"status": "Error", "message": f"{i} is required"},
        ...
        return JsonResponse({"status": "Success", "message": "Zapchast updated"}, status=status.HTTP_200_OK)
...
class Maxsulot_all(APIView):
    ...
    @staticmethod
    def post(request):
        required = ["zapchast_id", 'name_uz', "name_ru", 'description', "description_ru", "image",
                    'price', 'brand', "brand_image"]
        arr = {}
        for i in required:
            if i not in request.data:
                return JsonResponse({"status": "Error", "message": f"{i} is required"},
...
class OrdersView(APIView):
    @staticmethod
    def get(request):
        return query_to_data(Order.objects.all(), request)

    @staticmethod
    def post(request):
        required_fields = ["name", "phone_number", "maxsulot_id"]
        data = request.data
        missing_fields = [field for field in required_fields if field not in data]
        if missing_fields:
            return JsonResponse(
                {"status": "Error", "message": f"Required fields are missing: {', '.join(missing_fields)}"},
                status=status.HTTP_400_BAD_REQUEST)
    ...
    def put(request):
        try:
            ...
            return JsonResponse({"status": "Error", "message": "'order_id' must be an integer"},
                                status=status.HTTP_400_BAD_REQUEST)
        except Order.DoesNotExist:
            return JsonResponse({"status": "Error", "message": "Order does not exist"},
                                status=status.HTTP_404_NOT_FOUND)

        required = ['name', 'phone_number', 'maxsulot_id']
        arr = {}
        for i in required:
            if i not in request.data:
                return JsonResponse({"status": "Error", "message": f"{i} is required"},
        ...
        return JsonResponse({"status": "Success", "message": "Order updated"}, status=status.HTTP_200_OK)
